    ...
    for each in filtered:
        if len(filtered[each]) > 1 and each is not None:
            ...
            if storeid in result:
                ...
                result[storeid] = (str(len(filtered[each]))+' mods where NULL for the store ')
    return result


def main():
    findduplicate()
    print('please check "d://result.json" file for the result')
